...
from core.view.canvas.canvas_view import CanvasView
...
from core.view.canvas.canvas_view import EtatGraphicsObject
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, container):
        super().__init__()
        self.setWindowTitle("CORE Editor")
        ...
        self.router = Router()
        routes(self.router, self.canvas)

        # Chargement automatique de la dernière application utilisée
        self.applis_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../core/data/applis_gemma'))
    ...
    def charger_derniere_application(self):
        ...
        settings_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../core/data/settings.json'))   
        ...
        if last_app:
            file_path = os.path.join(self.applis_dir, last_app)
                ...
                try:
                    ...
                    for etat in etats:
                        ...
                        etat_item = self.canvas.scene.addItem(
                            __import__('core.view.canvas.canvas_view', fromlist=['EtatGraphicsObject']).EtatGraphicsObject(code, label, w, h)
                        )
    ...
    def sauvegarder_application(self):
        os.makedirs(self.applis_dir, exist_ok=True)
       
        dialog = QFileDialog(self)
        dialog.setWindowTitle("Sauvegarder l'application")
        dialog.setDirectory(self.applis_dir)
    ...
    def handle_charger_app(self):
        ...
        if not os.path.exists(self.applis_dir) or not any(f.endswith('.json') for f in os.listdir(self.applis_dir)):
